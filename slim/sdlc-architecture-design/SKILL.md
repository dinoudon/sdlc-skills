...
# Architecture, Design & Development (Slim)
...
| Code | UML class diagram (rarely needed) | Devs |
...
|-----------|----------|----------|--------------|
| Relational | ACID, complex joins | PostgreSQL, MySQL | Append-only logs |
| Document | Flexible schema, nested data | MongoDB, DynamoDB | Complex joins |
| Key-Value | Caching, sessions | Redis, DynamoDB | Range queries |
| Columnar | Analytics, time-series | Cassandra, ClickHouse | OLTP workloads |
| Search | Full-text search | Elasticsearch | Primary data store |
| Graph | Relationship-heavy queries | Neo4j, Neptune | Simple CRUD |

**Decision framework:** Start PostgreSQL (80% of workloads). Add Elasticsearch only if full-text search is core. Add Redis only if sub-ms latency or pub/sub needed. Add graph DB only if >30% queries traverse relationships. Justify each with measured bottleneck.
...
def get_order(order_id):
    ...
    redis.setex(f"order:{order_id}", 300, json.dumps(order))
    return order
...
**Cache stampede protection:** mutex/lock (only one rebuilds), probabilistic early expiration, stale-while-revalidate.
...
**Conflict Resolution:** LWW (latest timestamp wins, risk: data loss). CRDTs merge automatically (G-Counter, OR-Set, LWW-Register). Used by Redis CRDB, Riak, Yjs.

## Step 4: Code Architecture Patterns

### Clean Architecture

Dependencies point inward. Domain has zero imports from infra.
...
**Backstage (Spotify → CNCF):** Software Catalog (register services), Software Templates (scaffold from golden path), TechDocs (docs-as-code).
...
| Best for | Event streaming | Task queues | Low-latency, IoT | Multi-tenant streaming |

**Decision guide:** Event sourcing → Kafka/Pulsar. Task queues → RabbitMQ. Low-latency → NATS. Simple setup → NATS.
...
**Kappa preferred over Lambda.** Single stream processing layer. Reprocess by replaying event log.
...
| Failure handling | Each service handles own | Orchestrator triggers compensations |
...
// Pattern 2: Semantic compensation
...
**Don't use choreography for >5 steps.** **Don't mix saga state with domain state.**
...
| Traffic mgmt | Rich: fault injection | Basic: retries, timeouts | Full L7 |
| Observability | Kiali, Jaeger | Viz dashboard | Hubble |
| Best for | Enterprise, complex routing | Simple, resource-constrained | Kernel-level perf |

**Ambient Mode (Istio):** eliminates sidecars. ztunnel (L4) per-node daemon + optional waypoint proxy (L7) per-service. GA in Istio 1.22+.

**Decision guide:** Simple/fast → Linkerd. Complex enterprise → Istio. Kernel perf → Cilium.
...
| Best for | Enterprise support | Sidecar/service mesh | Full-featured OSS |
...
**Temporal:** Workflow (deterministic, replayed from event history), Activity (non-deterministic work, retried independently), Workflow replay (on crash, replays history + resumes from last completed activity).
...
**1. Edge-side rendering (ESR):** Render HTML at edge, personalize per user/region.
...
**3. Edge authentication / token validation:** Validate JWT at edge before hitting origin.

**4. Edge A/B testing / feature flags:** Split traffic at edge without origin round-trip.

**5. Edge API gateway:** Rate limiting, auth, request transformation at edge.

**6. Edge-side aggregation (BFF pattern):** Aggregate multiple backend APIs at edge.
...
**Principles:** Energy Efficiency (minimize energy per request), Carbon Awareness (shift compute to cleaner grid times/regions), Hardware Efficiency (maximize utilization, extend lifespan).
...
**Patterns:** Time-shifting (Carbon Aware SDK), Region-shifting (electricityMap API), Demand shaping (reduce precision during high-carbon periods). Measurement: Cloud Carbon Footprint (CCF), Green Metrics Tool.
